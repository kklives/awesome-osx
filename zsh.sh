    ...
    if [ -z "$(which ${1} 2>/dev/null | sed -n '/\/'${1}'/p')" ]; then
        if [ ! -z "$(echo $DISTRIBUTION | sed -n '/CentOS/p')" ]; then
            ...
            if [ -z "$(strings /lib64/libc.so.6 | sed -n '/GLIBC_2.18/p')" ]; then
                ...
                cd ${ZDOTDIR:-$HOME} && curl -L -O ${GLIBC_URL}
        ...
        *)
            ...
            cd ${ZDOTDIR:-$HOME} && rm -rf ${INSTALL_P}* && curl -L -O ${3}${INSTALL_P} && $sdpkg -i ${INSTALL_P}
            ;;
        esac
    fi
}
...
if [ -z "$(ls -a ${ZDOTDIR:-$HOME} | sed -n '/\.oh-my-zsh/p')" ]; then
    ...
    # change ~/.zshrc
    case $DISTRIBUTION in
    MacOS) sed -i '' 's/plugins=(git)/plugins=(git docker zsh-autosuggestions z kubectl history themes zsh_reload extract command-not-found colored-man-pages)/' ${ZSHRC} ;;
    *) sed -i 's/plugins=(git)/plugins=(git docker zsh-autosuggestions z kubectl history themes zsh_reload extract command-not-found colored-man-pages)/' ${ZSHRC} ;;
    esac
    ...
    echo_color yellow "${SIGN_2} Downloading other cli tools  ${SIGN_2}"
    brew install ack htop nmap p7zip pandoc ssh-copy-id telnet tree wget wrk html2text tldr speedtest-cli
    brew install findergo google-chrome iina open-in-code postman typora upic visual-studio-code switchhosts iterm2 --cask
    echo "alias ip='curl cip.cc'" >>${ZSHRC}
    echo "alias proxy='export http_proxy=http://127.0.0.1:1087;export https_proxy=http://127.0.0.1:1087;export ALL_PROXY=socks5://127.0.0.1:1080'" >>${ZSHRC}
    echo "alias unproxy='unset http_proxy https_proxy ALL_PROXY'" >>${ZSHRC}
